...
abstract final class GrpcHeaders {
  ...
  /// 无参 getter（T7 AnonymousAccount.delete 保留的 fawkes hack 兼容），
  /// 每次生成全新随机 sessionId；[newHeaders] 走 [_buildFawkes] 用派生 sessionId。
  static String get fawkes =>
      _buildFawkes(IdentityCoreGenerators.generateSessionId());

  static String _buildFawkes(String sessionId) => base64Encode(
    FawkesReq(
  ...
  static Map<String, String> newHeaders([
    ...
    return {
      ...
      'x-bili-fawkes-req-bin': _buildFawkes(identity.derived.sessionId),
      'x-bili-ticket': '',
  ...
  static String _normalizeBuvid({
    ...
    // 无账号回退路径：等价于旧 `_buvid => LoginUtils.buvid`
    // （LoginUtils.buvid = Pref.buvid 委托 guestBuvid）
    if (accessKey == null) {
